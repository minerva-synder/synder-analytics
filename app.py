...
# ---------------------------------------------------------------------------
# Support QA store (persisted to JSON file)
# ---------------------------------------------------------------------------
QA_FILE = os.path.join(os.path.dirname(__file__), "qa_tickets.json")
...
def qa_list():
    """List all QA tickets, optionally filtered by batch."""
    with QA_LOCK:
        data = _qa_load()
    batch = request.args.get("batch")
    tickets = data.get("tickets", [])
    if batch:
        tickets = [t for t in tickets if t.get("batch") == batch]
    return jsonify({"success": True, "tickets": tickets, "batches": data.get("batches", {})})


@app.route("/api/qa/tickets", methods=["POST"])
def qa_push():
    """Push one or more ticket analyses for QA review.

    Expects JSON:
    {
      "batch": "2026-02-26-premium",
      "tickets": [
        {
          "ticket_id": "42538476851",
          "ticket_url": "https://app.hubspot.com/...",
          "customer": "Vital Veda",
          "agent": "Lily Cabrera",
          "categories": ["Unresolved/Dropped Thread", "SLA Breach"],
          "status": "flagged",       // "flagged" | "ok" | "pending"
          "summary": "...",
          "suggested_fix": "...",
    ...
    batch_name = body.get("batch", datetime.now().strftime("%Y-%m-%d-%H%M"))
    ...
    with QA_LOCK:
        ...
        for t in body["tickets"]:
            ...
            t["review_status"] = t.get("review_status", "pending")  # pending | approved | revision_needed
            # Use ticket_id as unique key — update if exists
            existing = next((i for i, x in enumerate(data["tickets"]) if x.get("ticket_id") == t.get("ticket_id")), None)
            if existing is not None:
                # Preserve comments and review_status from existing
                t["comments"] = data["tickets"][existing].get("comments", []) + t.get("comments", [])
                if data["tickets"][existing].get("review_status") != "pending":
                    t["review_status"] = data["tickets"][existing]["review_status"]
                data["tickets"][existing] = t
            else:
                data["tickets"].append(t)

        data.setdefault("batches", {})[batch_name] = {
            "created_at": now,
            "ticket_count": len(body["tickets"]),
        }
        _qa_save(data)

    return jsonify({"success": True, "batch": batch_name, "count": len(body["tickets"])})


@app.route("/api/qa/tickets/<ticket_id>/comment", methods=["POST"])
def qa_comment(ticket_id):
    """Add a comment to a ticket. Expects {"text": "...", "author": "..."}"""
    body = request.get_json()
    ...
    with QA_LOCK:
        data = _qa_load()
        ticket = next((t for t in data["tickets"] if str(t.get("ticket_id")) == str(ticket_id)), None)
        if not ticket:
            return jsonify({"error": "ticket not found"}), 404
        ticket.setdefault("comments", []).append({
            "text": body["text"],
            "author": body.get("author", "Reviewer"),
...
@app.route("/api/qa/tickets/<ticket_id>/review", methods=["POST"])
def qa_review(ticket_id):
    """Set review status. Expects {"review_status": "approved"|"revision_needed"|"pending"}"""
    body = request.get_json()
    if not body or body.get("review_status") not in ("approved", "revision_needed", "pending"):
        return jsonify({"error": "review_status must be approved, revision_needed, or pending"}), 400
    ...
    with QA_LOCK:
        ...
        if not ticket:
            return jsonify({"error": "ticket not found"}), 404
        ticket["review_status"] = body["review_status"]
        ticket["reviewed_at"] = datetime.now().isoformat()
        _qa_save(data)
    return jsonify({"success": True})


@app.route("/api/qa/tickets/<ticket_id>", methods=["DELETE"])
def qa_delete(ticket_id):
    """Delete a ticket from QA."""
    with QA_LOCK:
        data = _qa_load()
        data["tickets"] = [t for t in data["tickets"] if str(t.get("ticket_id")) != str(ticket_id)]
        _qa_save(data)
    return jsonify({"success": True})
